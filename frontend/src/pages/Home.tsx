...
export const Home: React.FC = () => {
  ...
  const [exportStepInfo, setExportStepInfo] = useState<ExportStepInfo>({});
  ...
  const handleTableClick = (table: TableMetadata) => {
    const tableRef = table.schemaName
  ...
  // 统一导出入口：编排「获取查询结果」→「格式化数据」→「创建文件」，带可视化进度
  const handleExport = (format: ExportFormat) => {
    // 子任务 1：获取查询结果
    const validation = getQueryResultForExport(queryResult);
    ...
    const doExport = () => {
      ...
      // 步骤 0：获取查询结果
      setTimeout(() => {
        setExportStepInfo((prev) => ({
          ...prev,
          rowCount: validation.data.rowCount,
          format,
        }));
        setExportStep(1);
        ...
        // 步骤 1：格式化数据
        setTimeout(() => {
          const content = formatQueryResult(validation.data, format);
          const contentSize = formatBytes(new Blob([content]).size);
          setExportStepInfo((prev) => ({
            ...prev,
            contentSize,
          }));
          ...
          // 步骤 2：创建文件
          setTimeout(() => {
            const fileName = downloadFile(content, format);
            setExportStepInfo((prev) => ({
              ...prev,
              fileName,
            }));
            setExportStep(3);
            message.success(
              `Exported ${validation.data.rowCount} rows to ${format.toUpperCase()}`
            );
          }, 500);
        }, 500);
      }, 300);
    ...
    if (validation.status === "needs_confirm") {
      Modal.confirm({
        ...
        content: `You are about to export ${validation.rowCount.toLocaleString()} rows. This may take a while and consume memory. Continue?`,
        onOk: () => doExport(),
      });
    } else {
      doExport();
